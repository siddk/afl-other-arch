...
static u64 total_crashes,             /* Total number of crashes          */
           ...
           blocks_eff_select;         /* Blocks selected as fuzzable      */
                                      ...
                                      /* SIDD START                       */
static u64 total_bits_changed = 0;    /* Total number of bits changed     */
                                      /* SIDD END                         */
...
static u8 run_target(char** argv) {
  ...
  if (do_log_write) {
    current_log_entry->num_edges = count_bytes(trace_bits);
    log_entries[current_log_idx] = *current_log_entry;
    current_log_idx++;
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:a:P:f:m:t:T:dnCB:S:M:x:Q")) > 0)
    ...
    switch (opt) {
      ...
      // SIDD START
      case 'P':
        SAYF("Entering afl-other-arch case 'P'!\n");
